...
Output:
4'''
prob=int(input("enter number of problems MAX has:"))
travel=int(input("time taken to travel from home: "))
initial_time=240
total_time=initial_time-travel
count=0
for i in range(1,prob+1):
    solv_time=5*i
    if solv_time<total_time:
        total_time-=solv_time
        count=count+1
